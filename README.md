...
1. **Floating Superpowers Toolbar (Gemini Native Look & Feel)**:
   - Placed directly above the chat prompt input bar.
...
2. **Real Usage & Reset Timers (5-Hour & Weekly)**:
   - Interactive usage badge (`🕒 X%`) showing current usage quota.
   - On-demand floating popover card displaying:
     - *5-hour usage* and remaining time to reset (e.g. *Resets in 2h 20m*).
     - *Weekly usage* and weekly reset countdown (e.g. *Weekly resets in 2d 8h*).
     - Instant refresh button (↻ Refresh).

3. **Message Navigation (`^` and `v`)**:
   - Jump smoothly and instantly to previous or next user questions and Gemini answers without tedious manual scrolling.
...
6. **Bulk Delete Recent Conversations**:
